...
router.post('/subscribe', async (req, res) => {
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: '🌸 Welcome to MindBloom — Your Wellness Journey Starts Now',
  ...
  try {
    ...
    for (let i = 0; i < users.length; i += 50) {
      ...
      await Promise.allSettled(batch.map(user =>
        resend.emails.send({ from: FROM_EMAIL, to: user.email, subject, html })
      ));
    }
...
function getMorningEmail() {
  ...
  <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; background: #fffbf0; padding: 40px; border-radius: 16px;">
    ...
    <p style="color: #a8a29e; font-size: 13px;">You're receiving this because you subscribed to morning reminders. Update in settings anytime.</p>
  </div>`;
}
